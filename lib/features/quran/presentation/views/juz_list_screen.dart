...
class _JuzListScreenState extends ConsumerState<JuzListScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final filteredJuz = List.generate(30, (i) => i + 1).where((j) {
      final name = isUrdu ? QuranUtils.juzNamesArabic[j - 1] : QuranUtils.juzNamesEnglish[j - 1];
      return name.toLowerCase().contains(_searchQuery.toLowerCase()) || j.toString() == _searchQuery;
    }).toList();
          ...
          Expanded(
            child: filteredJuz.isEmpty
              ...
              itemBuilder: (context, index) {
                ...
                final juzEnglishName = QuranUtils.juzNamesEnglish[juzNumber - 1];
                final juzArabicName = QuranUtils.juzNamesArabic[juzNumber - 1];

                return InkWell(
                  onTap: () {
                    final Map<int, List<int>> juzData = Map<int, List<int>>.from(
                    ...
                    int calculatedPage = quran.getPageNumber(startingSurah, startingVerse);

                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => MushafViewScreen(initialPage: calculatedPage),
                      ),
                    );
                  },
